...
template<any_span S>
Owned<S> Owned<S>::allocate( size_t size )
...
template<any_span S>
Owned<S> Owned<S>::map( size_t size )
  requires( not std::is_const_v<element_type> )
{
...
template<any_span S>
Owned<S> Owned<S>::from_file( const std::filesystem::path path )
...
template<any_span S>
void Owned<S>::to_file( const std::filesystem::path path )
  requires std::is_const_v<element_type>
{
...
template<any_span S>
Owned<S>::Owned( Owned<std::span<value_type>>&& original )
  requires std::is_const_v<element_type>
  : span_( original.span() )
...
template<any_span S>
Owned<S>& Owned<S>::operator=( Owned<std::span<value_type>>&& original )
  requires std::is_const_v<element_type>
{
  span_ = original.span();
  allocation_type_ = original.allocation_type();
