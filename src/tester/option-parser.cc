...
OptionParser::Option::Option( char short_name_,
...
void OptionParser::AddArgument( const std::string& name, ArgumentCount count, const Callback& callback )
{
...
{
  Option option(
    short_name, long_name, std::string(), HasArgument::No, help, [callback]( const char* ) { callback(); } );
  AddOption( option );
}

void OptionParser::AddOption( const char* long_name, const char* help, const NullCallback& callback )
{
  Option option( '\0', long_name, std::string(), HasArgument::No, help, [callback]( const char* ) { callback(); } );
  AddOption( option );
}
...
{
  Option option( short_name, long_name, metavar, HasArgument::Yes, help, callback );
  AddOption( option );
}
...
void OptionParser::ParseError( std::string error_msg )
{
...
void OptionParser::HandleArgument( size_t* arg_index, const char* arg_value )
{
  if ( *arg_index >= arguments_.size() ) {
    ParseError( std::string( "unexpected argument " ) + arg_value );
    return;
  }
    ...
    if ( arg[1] == '-' ) {
      ...
      for ( size_t j = 0; j < options_.size(); ++j ) {
        ...
        if ( !option.long_name.empty() ) {
          int match_length = Match( &arg[2], option.long_name, option.has_argument );
      ...
      if ( best_count > 1 ) {
        ParseError( std::string( "ambiguous option " ) + arg );
        continue;
      } else if ( best_count == 0 ) {
        ParseError( std::string( "unknown option " ) + arg );
        continue;
      }
      ...
      if ( best_option.has_argument ) {
        if ( arg[best_length + 1] != 0 &&    // This byte is 0 on a full match.
          ...
          if ( i + 1 == argc || argv[i + 1][0] == '-' ) {
            ParseError( std::string( "option requires argument --" ) + best_option.long_name );
            continue;
          }
          ++i;
      ...
      for ( int k = 1; arg[k]; ++k ) {
        ...
        for ( const Option& option : options_ ) {
          if ( option.short_name && arg[k] == option.short_name ) {
            ...
            if ( option.has_argument ) {
              ...
              if ( arg[k + 1] != '\0' ) {
                ParseError( std::string( "option requires argument -" ) + option.short_name );
                break;
              }
        ...
        if ( !matched ) {
          ParseError( std::string( "unknown option -" ) + arg[k] );
          continue;
        }
      }
    }
  ...
  if ( !arguments_.empty() && arguments_.back().handled_count == 0 ) {
    for ( size_t i = arg_index; i < arguments_.size(); ++i ) {
      if ( arguments_[i].count != ArgumentCount::ZeroOrMore ) {
        ParseError( std::string( "expected argument " ) + arguments_[i].name );
      }
    }
  }
}
