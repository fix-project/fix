...
std::pair< std::string, std::string > wasm_to_c( std::string & wasm_content ) 
{
  Errors errors;
  Module module;
  const bool kStopOnFirstError = true;
  ...
  ReadBinaryOptions options( s_features, s_log_stream.get(),
                              s_read_debug_names, kStopOnFirstError,
                              kFailOnCustomSectionError );
  result = ReadBinaryIr(s_infile.c_str(), wasm_content.c_str(), wasm_content.size(),
                        options, &errors, &module);
  if (Succeeded(result)) {
    if (Succeeded(result)) {
      ValidateOptions options(s_features);
  ...
  return { c_stream.ReleaseOutputBuffer()->data, h_stream.ReleaseOutputBuffer()->data };
}
