...
int main( int argc, char** argv )
{
  ...
  const Type* tree = make_terminal( "Tree", 240 + 1 ); // hash + is_local
  const Type* tag = make_terminal( "Tag", 240 + 1 );
  const Type* tref = make_terminal( "TreeRef", 240 + 1 );
  const Type* tagref = make_terminal( "TagRef", 240 + 1 );

  const Type* treeish = make_sum( "Treeish", 2, tree, tag );
  const Type* treeishref = make_sum( "TreeishRef", 2, tref, tagref );
  ...
  const Type* value = make_sum( "Value", 4, blob, bref, treeish, treeishref );

  const Type* application = make_wrapper( "Application", tree );
  const Type* identification = make_wrapper( "Identification", value );
  const Type* selection = make_wrapper( "Selection", tree );
  ...
  const Type* fix = make_sum( "Fix", 3, encode, thunk, value );
