...
{
  auto state = continuation_.empty() ? collector_.get_think_identity( thunk )
                                     : get<ThinkTag>( continuation_.back().value() );
  ...
  while ( true ) {
    if ( auto x = state.result.try_into<Thunk>(); x.has_value() ) {
      auto newthinktag = force( x.value() );
      if ( newthinktag.has_value() ) {
        state = collector_.think_transitive( state, newthinktag.value() ).value();
      } else {
