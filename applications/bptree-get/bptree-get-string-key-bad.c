...
__attribute__( ( export_name( "_fixpoint_apply" ) ) ) externref _fixpoint_apply( externref combination )
{
  ...
  if ( fixpoint_is_blob( keys_h ) ) {
    ...
    if ( isleaf ) {
      ...
      externref all_next_level = create_selection_thunk_range( nextlevel, 0, 16777218 + 1 );
      set_rw_table_0( 0, get_ro_table_0( 0 ) );
      set_rw_table_0( 1, get_ro_table_0( 1 ) );
      set_rw_table_0( 2, create_shallow_encode( all_next_level ) );
